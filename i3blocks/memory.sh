...
if [[ "${DISPLAY}" != "perc" ]]; then
  ...
  if [[ "${MEMINFO}" -ge "${ONE_TB}" ]]; then
    MEMINFO=$(echo "scale=3;${MEMINFO}/${ONE_TB}" | bc -l)"T"
  elif [[ "${MEMINFO}" -ge "${ONE_GB}" ]]; then
    MEMINFO=$(echo "scale=2;${MEMINFO}/${ONE_GB}" | bc -l)"G"
  elif [[ "${MEMINFO}" -ge "${ONE_MB}" ]]; then
    MEMINFO=$(echo "scale=1;${MEMINFO}/${ONE_MB}" | bc -l)"M"
  else
    MEMINFO=$(echo "scale=0;${MEMINFO}/${ONE_KB}" | bc -l)"K"
  fi
fi
