...
# Prompt on left side
if [ "$SSH_CONNECTION" != "" ]; then remote_indicator="%B"; else remote_indicator=""; fi
if [ `whoami` = root ]; then root_indicator="[%{$fg[red]%}R%{$reset_color%}]"; else root_indicator=""; fi
PROMPT="[$remote_indicator%{$fg[yellow]%}%M%{$reset_color%}]$root_indicator:%{$fg[magenta]%}%~%{$reset_color%} > "
